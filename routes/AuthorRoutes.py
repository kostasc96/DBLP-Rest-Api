...
from persistence.queries import query1, query2, query3, query4, query5, query6, query6_2, query8, query9, query10, \
    query11, query13, query14, query16, query18, query12, query15
...
def get_topk_query12():
    ...
    k = int(request.args.get('k'))
    data = graph.run(query12, name=journal_name, year=year, pos=position, k=k).data()
    return jsonify(data)
...
def get_query16():
    k = int(request.args.get('k'))
    data = graph.run(query16, k=k).data()
    return jsonify(data)


@route1.route('/query18')
def get_query18():
    data = graph.run(query18).data()
    return jsonify(data)
